...
function goBack () {
  ...
  if (len > 1) {
    router.push("/panorama-work/" + panoramaWork.history[len - 2].hashId);
...
function goToActive (row) {
  if (row.type == 1) {
    ...
    router.push("/panorama-work/" + hashId + "?name=" + row.name);
  } else if (row.type == 0) {
...
function editPanoramaWork (id) {
  try {
    ...
    router.push("/panorama-work-edit/" + id);
  } catch (e) {
    console.log(e);
  }
}
function showPanoramaWork (id) {
  try {
    ...
    window.open("/panorama-work-show/" + id, "全景作品预览", features);
  } catch (e) {
    console.log(e);
  }
}
