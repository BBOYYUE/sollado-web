...
function track_mouse_interval_callback () {
  ...
  editorStore.updateActiveHotspotPosition({
    atv: atv,
    ath: ath
  })
      ...
      <el-tabs>
        <el-tab-pane :label="props.alias">
          <div class="flex flex-row flex-wrap">
            <box size="sm" marign="none" class="mt-1 w-full" :bgColor="hash_id == activeData.hash_id ? 'bg-blue-200' : ''"
        ...
        <el-tab-pane :label="groupAlias">
          <el-tabs tab-position="right" @tab-click="groupTabClick">
            <el-tab-pane :key="group.hash_id" :label="group.name" v-for="group in groupData" :name="group.hash_id">
              <box size="sm" marign="none" class="mt-1  w-full" v-for="hash_id in group[dataTypePlural]" :key="hash_id"
